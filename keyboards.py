...
def ceo_panel():
    ...
    b.row(
        types.InlineKeyboardButton(text="🔙 Orqaga", callback_data="back_main")
    )
    return b.as_markup()

def subscribe_btn(channel):
    b = InlineKeyboardBuilder()
    # lstrip('@') orqali kanaldagi @ belgisi bo'lsa ham muammosiz url yaratiladi
    b.row(
        types.InlineKeyboardButton(text="📢 Kanalga obuna bo'lish", url=f"https://t.me/{channel.lstrip('@')}")
    )
    b.row(
